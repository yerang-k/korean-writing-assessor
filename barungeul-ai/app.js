...
function renderSubmissionsList() {
    ...
    filteredSubmissions.forEach((sub, idx) => {
        ...
        row.innerHTML = `
            ...
            <td style="padding: 12px 16px; font-weight: bold; color: var(--color-indigo, #6366f1);">${sub.총점 || '0'}점</td>
...
function showSubmissionDetail(sub) {
    ...
    document.getElementById('sub-detail-score').textContent = `${sub.총점 || '0'} / 100`;
...
function downloadSubmissionAsWord(sub) {
    ...
    <body>
        ...
        <h2>1. 종합 채점 결과: <span style="color:#4f46e5;">${sub.총점 || '0'} / ${totalMax || '100'}점</span></h2>
...
function downloadResultAsWord(result) {
    ...
    <body>
        ...
        <h2>1. 종합 채점 결과: <span style="color:#4f46e5;">${totalAcquired} / ${totalMax}점</span></h2>
        <table class="rubric-table">
            <thead>
                <tr>
                    <th style="width: 30%;">평가 항목</th>
